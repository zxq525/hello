...
class PhpUpImageWater
{
    ...
    function PhpUpImageWater($groundImage = "demo.jpg", $waterPosType = 5)
    {
    ...
    function setGroundImageInfo()
    {
        ...
        if($this->groundImageWidth < 30 or $this->groundImageHeight < 30)
        {
            $this->throwError("TooSmall");
        }
    }

    function setWaterTextInfo($waterText = "phpup.com",$waterTextColor = "#000000", $waterTextSize = "5", $waterTextFont = "fonts.ttf")
    {
        $this->waterType     = 0;
    ...
    function putWateredImage($extFileName)
    {
        ...
        {
            case 1:
                imagegif($this->groundImageHandle, $extFileName.basename($this->groundImage));
                break;
            case 2:
                imagejpeg($this->groundImageHandle, $extFileName.basename($this->groundImage));
                break;
            case 3:
                imagepng($this->groundImageHandle, $extFileName.basename($this->groundImage));
                break;
            default:
                $this->throwError("NonType");
        }       
...
<?php
/*
$h = new PhpUpImageWater("a.jpg",8);以前此处不能加目录，我已经修改，加了basename()
//$h->setWaterImageInfo("shui.gif");
$h->setWaterTextInfo("中国","#000000","100");
$h->makeWater("test_");
*/
?> 
