...
if(!checkint($classid)){
	...
	$class=$db->fetch_first("SELECT * FROM {$dbtablepre}article_class where id=$classid AND classpage='infoclass' order by sort_order asc, id asc limit 1");
	if(!$class){
		jsalert("没有找到相关信息，请重试；","back","window");
	}
...
?>
      <li><div class="topic"><a href="infoclass_detail.php?id=<?php echo $rs['id'];?>" target="_blank"><?php echo $rs['topic'];?></a></div><div class="date"><?php echo date('Y-m-d',$rs['joindate']);?></div></li>
<?php
	$i++;
}
?>
