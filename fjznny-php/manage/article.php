...
$urlquery = "&s_topic=". urlencode($s_topic) ."&s_keyword=". urlencode($s_keyword)."&s_classid=". urlencode($s_classid)."&s_isgood=". urlencode($s_isgood);
...
if($action=="add"){
	if($act=="addsave"){
		if(!checkint($classid)){$errmsg.="@@请选择所属分类；\\n";}
		if($topic==""){$errmsg.="@@请输入信息标题；\\n";}
		if(!checkdatetime($joindate))$errmsg.="@@请选择正确的添加时间；\\n";
		
		if(!checkint($isgood)){$isgood=0;}
		
		if(!$errmsg){
			$rs=$db->fetch_first("select classname from {$dbtablepre}article_class where id='".$classid."'");
			if($rs){
		...
		if(!$errmsg){
			...
			$hitnum=0;
			$db->query("insert into {$dbtablepre}article(classid,topic,keyword,author,source,infoimg,infointro,content,isgood,joindate,hitnum)values($classid,'$topic','$keyword','$author','$source','$infoimg','$infointro','$content','$isgood','$joindate','$hitnum')");
...
<?=select_catalog($s_classid,0,1) ?>
  ...
  <tr>
    <td height="24" align="right"><strong>关 键 字：</strong></td>
    <td><input name="keyword" type="text" class="input" size="30" value="<?php echo $val['keyword'];?>"></td>
  </tr>
  <tr>
    <td height="24" align="right"><strong>发布作者：</strong></td>
    <td><input name="author" type="text" class="input" size="30" value="<?php echo $val['author'];?>"></td>
  </tr>
  <tr>
    <td height="24" align="right"><strong>信息来源：</strong></td>
...
<?php echo select_catalog($s_classid,0,1) ?>
      </select>
...
if(checkint($s_classid)){
	$childidarr=$db->result_first("select childidarr from {$dbtablepre}article_class where id=$s_classid");
	if($childidarr){
...
$query = "select A.*,B.classname from {$dbtablepre}article as A left join {$dbtablepre}article_class as B on A.classid=B.id where 1 $sql2 order by A.joindate desc,A.id desc";
...
?>
  <tr>
    ...
    <td align="left">&nbsp;<a href="?mp=<?php echo $mp;?>&action=edit&id=<?php echo $rs["id"].$urlquery;?>" title="编辑信息" class="cboxframe"><?php echo getcutstr($rs["topic"],40);?></a></td>
...
if($action=="edit"){
	...
	if($act=="editsave"){
		...
		if(!$errmsg){
			...
			$joindate=strtotime($joindate);
			$db->query("update {$dbtablepre}article set classid=$classid,topic='$topic',keyword='$keyword',author='$author',source='$source',infoimg='$infoimg',infointro='$infointro',content='$content',isgood=$isgood,joindate='$joindate' where id=".$val['id']."");
...
<form name="Form1" action="?mp=<?php echo $mp;?>&action=edit&act=editsave&id=<?php echo $id;?>&page=<?php echo $page;?><?php echo $urlquery;?>" method="post" enctype="multipart/form-data" target="postpanel">
  ...
  <tr>
    ...
    <td><select name="classid">
<option value="">--请选择分类--</option>
<?=select_catalog($val['classid'],0,1) ?>
</select></td>
  </tr>
  <tr>
    <td height="24" align="right"><strong><img src="images/icon01.gif" align="absmiddle" />信息标题：</strong></td>
  ...
  <tr>
    <td height="24" align="right"><strong>信息图片：</strong></td>
    <td><input name="infoimg" type="file" class="input" value="<?php echo $val['infoimg'];?>" size="30" style="height:22px; margin-right:50px;" />
    <?php
    if($val['infoimg']){
