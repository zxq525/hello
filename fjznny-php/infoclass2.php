...
if(!checkint($classid)){
	...
	$class=$db->fetch_first("SELECT * FROM {$dbtablepre}article_class where id=$classid AND classpage='infoclass2' order by sort_order asc, id asc limit 1");
	if(!$class){
		jsalert("没有找到相关信息，请重试；","back","window");
	}
...
$pagecls = new pagecls($num,15,$page,"?classid=".$class['id']);
$query=$db->query("$query LIMIT {$pagecls->startrecord},{$pagecls->pagesize}");
$i=0;
while($rs = $db->fetch_array($query)) {
?>
      <li><div class="topic"><a href="infoclass2_detail.php?id=<?php echo $rs['id'];?>" target="_blank"><?php echo $rs['topic'];?></a></div><div class="date"><?php echo date('Y-m-d',$rs['joindate']);?></div></li>
<?php
	$i++;
}
?>
